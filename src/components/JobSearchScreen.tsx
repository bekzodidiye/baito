...
import { JobCardSkeleton } from './search/JobCardSkeleton';
...
export const JobSearchScreen: React.FC = () => {
  ...
  // Reset pagination to 10 and trigger skeleton screen loader when filters or sort change
  useEffect(() => {
    setVisibleCount(10);
    setIsSearching(false); // Temporarily disabled as requested
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isSearching ? (
            Array.from({ length: 4 }).map((_, idx) => (
              <JobCardSkeleton key={`skeleton-mob-${idx}`} idx={idx} />
            ))
          ) : (
          ...
          <div className="flex flex-col gap-3.5">
            {isSearching ? (
              Array.from({ length: 3 }).map((_, idx) => (
                <JobCardSkeleton key={`skeleton-desk-${idx}`} idx={idx} />
              ))
            ) : (
              sortedJobs.slice(0, visibleCount).map((job, idx) => (
                <JobCardItem 
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <a href={`https://maps.google.com/?q=${encodeURIComponent(activeDesktopJob.location)}`} target="_blank" rel="noreferrer" className="text-[10px] text-brand-primary hover:underline font-bold flex items-center gap-0.5">
              ...
              <button
                ...
                onClick={() => {
                  const success = applyToJob(activeDesktopJob.id);
                  if (success && selectedJob && selectedJob.id === activeDesktopJob.id) {
