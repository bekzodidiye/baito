...
const REGIONS_LIST: RegionListItem[] = [
  { id: "Toshkent_shahri", name: "Toshkent shahri", count: "Poytaxt, 4,500+ bo'sh ish o'rni", isTashkentCity: true },
...
export const RegionSelector: React.FC = () => {
  ...
  return (
    <div id="region-selector-modal" className="fixed inset-0 z-[10000] bg-brand-background text-brand-text flex flex-col min-h-screen">
      {/* TopAppBar Component */}
      <header className="fixed top-0 left-0 w-full z-50 bg-white border-b border-brand-outline-variant flex justify-between items-center px-4 h-16 max-w-full">
        <button 
          id="btn-back-region"
          onClick={handleBackClick}
          aria-label="Back" 
          className="text-brand-text-variant hover:bg-brand-surface-low transition-colors active:opacity-80 p-2 rounded-full flex items-center justify-center cursor-pointer"
          type="button"
        >
          <ArrowLeft size={22} />
        </button>
      ...
      <main className="flex-1 mt-16 px-4 py-4 overflow-y-auto max-w-xl mx-auto w-full">
        ...
        <div className="relative mb-6">
          ...
          <input 
            ...
            className="block w-full pl-10 pr-10 py-3 border border-brand-outline-variant rounded-xl bg-white text-sm font-sans text-brand-text placeholder:text-brand-outline focus:outline-hidden focus:ring-2 focus:ring-brand-primary focus:border-transparent transition-all" 
          ...
          {!selectedRegion && searchText ? (
            searchResults.length > 0 ? (
              searchResults.map((match, idx) => (
                <button 
                  ...
                  className="w-full bg-white p-4 rounded-xl shadow-xs hover:shadow-md transition-all flex items-center group text-left cursor-pointer active:scale-99"
                >
            ...
            <>
              ...
              <button 
                ...
                className="w-full bg-blue-50/50 hover:bg-blue-50 border border-blue-100/50 p-4 rounded-xl shadow-xs transition-all flex items-center group text-left cursor-pointer active:scale-99"
              >
            ...
            <>
              ...
              <button 
                ...
                className="w-full bg-emerald-50/50 hover:bg-emerald-50 border border-emerald-100/50 p-4 rounded-xl shadow-xs transition-all flex items-center group text-left cursor-pointer active:scale-99"
              >
