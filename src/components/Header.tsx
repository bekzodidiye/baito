...
export const Header: React.FC = () => {
  const { currentScreen, setCurrentScreen, setDrawerOpen, unreadNotificationsCount, setShowRegionSelector } = useApp();
  ...
  const renderMobileActions = () => {
    switch (currentScreen) {
      ...
      case 'xabarlar':
        return (
          <button className="p-2 cursor-pointer text-brand-primary hover:bg-brand-surface-low rounded-full transition-colors">
            <Search size={22} />
          </button>
        );
      case 'xarita':
  ...
  return (
    <>
      {/* Mobile Top App Bar */}
      {currentScreen !== 'chat' && (
        <header className="flex md:hidden justify-between items-center px-4 h-14 w-full z-50 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)] fixed top-0 left-0">
          <button
      ...
      {/* Desktop Top Nav */}
      <header className="hidden md:flex justify-between items-center px-6 h-16 w-full z-50 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)] sticky top-0">
