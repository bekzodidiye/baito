...
export const useJobMarkers = ({
  ...
  useEffect(() => {
    ...
    displayedJobs.forEach(job => {
      ...
      const jobHtml = `
        <div class="flex flex-col items-center justify-center cursor-pointer select-none transition-all duration-300 ease-out origin-bottom hover:scale-115 transform-gpu" style="width: 50px; height: 44px;">
          <div class="relative flex items-center justify-center rounded-full border-2 bg-white shadow-sm transition-all duration-300 ${
            isSelected ? 'border-[#000666] scale-110 z-30 shadow-md' : 'border-[#c6c5d4] z-20'
    ...
    if (userLocation && userGroup) {
      const userHtml = `
        <div class="relative flex items-center justify-center" style="width: 40px; height: 40px;">
          <div class="w-4 h-4 bg-[#000666] rounded-full border-2 border-white shadow-lg relative z-10"></div>
          <span class="absolute w-8 h-8 rounded-full bg-[#000666]/60 animate-ping opacity-40"></span>
        </div>
      `;
  ...
  }, [activeCluster, jobs, selectedJob, userLocation, filterLocation, mapInstanceRef, markerGroupRef, userGroupRef, getDisplayedJobs, setIsPanelExpanded, setSelectedJob]);
};
